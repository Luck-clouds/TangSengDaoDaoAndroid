# 卿航IM（[官网](https://qinghangim.com/ "官网")）
...
卿航IM实现聊天功能。Demo已实现`文本`，`图片`，`语音`，`名片`，`emoji`，群聊`@某人`，消息`链接`，`手机号`，`邮箱`识别等功能。聊天设置支持`名称修改`，`头像修改`，`公告编辑`，`消息免打扰`，`置顶`，`保存到通讯录`，`聊天内昵称`，`群内成员昵称显示`等丰富的设置功能。由于demo是模块化开发，开发者可完全按自己的开发习惯进行二次开发。

### 卿航IM特点

- #### **永久保存消息** 卸载卿航IM后下次安装登录后，可查看以前的聊天记录
- #### **超大群** 卿航IM群聊人数无限制，万人群进入聊天完全不卡，消息正常收发
- #### **实时性** 所有操作实时同步，app已读消息，web/pc可实时更改状态
- #### **扩展性强** 卿航IM现已框架可轻松支持消息的已读未读回执，消息点赞，消息回复功能
- #### **开源** 卿航IM100%开源，商业开发无需授权可直接使用

### 项目模块
卿航IM是模块化开发，不限制开发者编码习惯。以下是对各个模块的说明
...
**注意这里只是介绍如何将自定义的消息item展示在消息列表中，消息model的实现可查看项目中的消息实现**

卿航IM实现自定义消息Item也十分简单。只需要实现两步即可
...
override fun setData(
 ...
 ```
> 这里的`WKCardContent`消息对象是卿航消息对象的实现，所有自定义消息model都需要基于项目现有消息结构扩展。
...
如果扫描错误可通过 [安装地址](http://www.pgyer.com/qinghangim "文档") 下载

### 许可证
卿航 使用 Apache 2.0 许可证。有关详情，请参阅 LICENSE 文件。
