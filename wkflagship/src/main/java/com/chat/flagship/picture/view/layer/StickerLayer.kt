...
import com.chat.flagship.picture.util.Vector2D
...
class StickerLayer(
    ...
    private fun adjustAngle(degrees: Float) {
        ...
        if (!degrees.isNaN()) {
            currRotation = initRotation + rotation
        }
    }
    ...
    private fun parentScaleX(): Float = parentMatrix.values()[0]
    private fun parentScaleY(): Float = parentMatrix.values()[4]
    private fun parentTranslateX(): Float = parentMatrix.values()[2]
    private fun parentTranslateY(): Float = parentMatrix.values()[5]
}
