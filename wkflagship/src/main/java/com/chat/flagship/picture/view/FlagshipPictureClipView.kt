...
class FlagshipPictureClipView @JvmOverloads constructor(
    ...
    private var paddingBottomPx = 0f
    ...
    fun setClipPadding(top: Float, bottom: Float) {
        paddingTopPx = top
    ...
    fun saveBitmap(): Bitmap {
        ...
        canvas.drawRect(clipRectF, localPaint)
        localPaint.xfermode = PorterDuffXfermode(PorterDuff.Mode.SRC_IN)
        bitmapRectF.offset(left, top)
        canvas.drawBitmap(bitmap, null, bitmapRectF, localPaint)
        return clipBitmap
    }
    ...
    private fun computeBitmapRectF() {
        ...
        val maxClipTop = DRAG_WIDTH + paddingTopPx
        val maxClipRight = viewWidth - DRAG_WIDTH
        val maxClipBottom = viewHeight - DRAG_WIDTH - paddingBottomPx
