...
public class GroupModel extends WKBaseModel {
    ...
    public void getGroupDetail(String groupNo, final IGroupInfo listener) {
        request(createService(GroupService.class).getGroupDetail(groupNo), new IRequestResultListener<>() {
            @Override
            public void onSuccess(GroupEntity groupEntity) {
                if (listener != null) {
                    listener.onResult(HttpResponseCode.success, "", groupEntity);
                }
            }
